...
q-p 字母行上滑可输出 `!@#$%^&*()`，下滑可输出 `1234567890`（也可通过长按输出数字）；a-l 字母行上滑可输出 `` `~_+{}|:"? ``，下滑可输出 `-=[]\;'I`；a、x、c、v 长按对应**全选**、**剪切**、**复制**、**粘贴**；z、m 长按对应**反查**、**快捷功能**；逗号键上滑输出 `<`，长按进入**表情**界面；句号键上滑输出 `>`，长按进入**剪贴板**；删除键上滑可实现**一键清除**，下滑可**撤销**一键清除。
